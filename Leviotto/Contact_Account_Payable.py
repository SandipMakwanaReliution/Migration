...
with old_registry.cursor() as cr :
    ...
    for partner in partners :
        if partner.property_account_payable_id :
            old_partner_account_map[partner.id] = partner.property_account_payable_id.code
            # Optional print
            print(f"Old Partner ID {partner.id} -> Old Receivable Account ID {partner.property_account_payable_id.code}")
...
with new_registry.cursor() as cr :
    ...
    for old_partner_id, old_account_id in old_partner_account_map.items() :
        ...
        print(f"✅ Updated partner {new_partner.name} (old_id {old_partner_id}) with new receivable account {new_account.code}")